...
- **Symlinks**: no se siguen. Un symlink en el lugar de una copia o de una carpeta de copias se informa como conflicto; `--force` lo reemplaza por la copia.
...
| **Conflictos** | Copias editadas a mano; archivos no administrados que difieren; archivos extra dentro de una skill administrada; un archivo o un symlink donde debería haber una carpeta o una copia (o al revés); `skill.md` con otras mayúsculas; rutas que git tiene registradas con otras mayúsculas | Sí |
...
- **Archivo o symlink donde debería haber una carpeta o una copia** (por ejemplo, los symlinks de Specboot, que en Windows sin symlinks se clonan como archivos de texto): borralo, o usá `--force` para que el script lo reemplace por la copia.
